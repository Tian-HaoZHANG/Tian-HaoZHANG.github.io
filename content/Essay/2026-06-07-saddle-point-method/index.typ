...
In order to evaluate this integral, we deform $C$ such that it passes through the saddle points of $S$ (where $S'(z) = 0)$ in the steepest descent directions. The idea of the saddle point method is that, as $lambda -> +infinity,$ the main contribution to the integral comes from the neighborhood of the saddle points. For simplicity, let's assume that $S$ has only one saddle point $z = z_0;$ if $S$ has multiple saddle points, we need only to sum the contributions from each. Taylor expanding $S$ about $z_0$ and noting that the Gaussian profile has width $abs(z - z_0) = order(lambda^(-frac(1, 2, style: "horizontal"))),$ we have
$
