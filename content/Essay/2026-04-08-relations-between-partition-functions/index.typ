...
#let description = "一个神奇的观察：不同配分函数之间通过通过（类） Laplace 变换相联系。"
#show: template.with(
  title: title,
...
哈密顿量 $cal(H)$ 是概率空间$(Omega = M, cal(F) = 2^(M), P_M)$上的随机#footnote[“随机变量”既不“随机”，也不是“变量”；它是一个确定的对应法则，其随机性完全来自于取样的随机性。在这里，系统的微观态具有随机性，因此哈密顿量成为了随机变量。]变量，其特征函数按定义为
...
其中 $sum_(mu|bm(x))$ 表示在固定 $bm(x)$ 的条件下对 $mu$ 求和；同样，这里写出了配分函数的自变量作为强调：Gibbs 配分函数的自变量是广义力 $bm(J),$ 正则配分函数则是广义坐标 $bm(x).$ 如果广义坐标的取值是连续的，上式就是 Laplace 变换；例如理想气体 $J = -p, x = V,$
